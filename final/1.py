...
def draw_roc_curve(image: Image.Image, ground_truth_image: Image.Image):
    ...
    for thresh in tqdm(thresholds):
        binary_image = region_growing(blurred_image, seed_point=seed_point, threshold=thresh)
        tp = np.sum((binary_image == 255) & (gt_array == 255))
        tn = np.sum((binary_image == 0) & (gt_array == 0))
        fp = np.sum((binary_image == 255) & (gt_array == 0))
        fn = np.sum((binary_image == 0) & (gt_array == 255))
    ...
    blurred_image = image.copy().filter(ImageFilter.GaussianBlur(radius=2))
    for thresh in tqdm(thresholds):
        binary_image_combined = threshold_and_region_growing(image, threshold=thresh)
...
def get_safe_ocean_seeds(image: Image.Image):
    ...
    blurred_image = copy_image.filter(ImageFilter.GaussianBlur(radius=3))
...
def threshold_and_region_growing(image: Image.Image, threshold: int):
    ...
    segmented_images = []
    for y in tqdm(range(0, binary_seeds.shape[0], 50), desc="Processing rows for seeds"):
        for x in range(0, binary_seeds.shape[1], 50):
            if binary_seeds[y, x] == 255:
                # print(f"Seed found at: ({x}, {y})")
                segmented_image = region_growing(image, seed_point=(x, y), threshold=threshold)
