...
# Load video
video_path = "./Videos/bottom_2.avi"
...
prev_des = None
prev_time = None
angle_speeds = deque(maxlen=10)

# Smoothing variables for readability
UPDATE_INTERVAL = 2.0          
last_update_time = 0.0
display_avg_w = 0.0
display_sec_per_rot = 0.0
...
while True:
    ...
    # Angular velocity
    if prev_kp is not None and prev_des is not None and prev_time is not None:
        matches_rt = bf.knnMatch(prev_des, des, k=2)
        good_rt = [m for m,n in matches_rt if m.distance < 0.75*n.distance]
        ...
        for m in good_rt[:20]:
            ...
            a2 = math.degrees(math.atan2(y2 - y0, x2 - x0))

            da = a2 - a1
            if da > 180: da -= 360
            if da < -180: da += 360

            angle_list.append(da)

        if len(angle_list) > 3:
            mean_da = np.mean(angle_list)
            dt = time_now - prev_time
            if dt > 0:
                omega = mean_da / dt
                angle_speeds.append(omega)

    # Output smoothing
    if time_now - last_update_time > UPDATE_INTERVAL:
        display_avg_w = np.mean(angle_speeds) if len(angle_speeds)>0 else 0
        display_sec_per_rot = 360 / max(abs(display_avg_w), 1e-6)
        last_update_time = time_now

    avg_w = display_avg_w
    sec_per_rot = display_sec_per_rot
    ...
    draw_text_with_box(vis,f"Velocity: {avg_w:.2f} deg/s",(tx,200))
    draw_text_with_box(vis,f"Sec/Rotation: {sec_per_rot:.2f}",(tx,250))
