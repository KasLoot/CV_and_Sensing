...
def colour_thresholding_hough_circle(hsv_image, gray_image, lower_bound, upper_bound, param2=30, min_radius=300, max_radius=500):
    ...
    hough_mask = hough_circle_mask(gray_image, param2, min_radius, max_radius)
    combined_mask = cv2.bitwise_or(colour_mask, hough_mask)

    def format_coord(x, y):
        col = int(x + 0.5)
        row = int(y + 0.5)
        if col >= 0 and col < hsv_image.shape[1] and row >= 0 and row < hsv_image.shape[0]:
            return f'x={x:.1f}, y={y:.1f}, HSV={hsv_image[row, col]}'
